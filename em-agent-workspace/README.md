...
Five new schemas created in `team-contracts/schemas/`:
...
pytest em-agent-workspace/tests/test_nodes.py::TestCapacityAnalysis -v

# Run with coverage
pytest em-agent-workspace/tests/test_nodes.py --cov=em_agent_workspace
```
...
Five new schemas created for EM workflow (in `team-contracts/schemas/`):
...
See `team-contracts/README.md` for complete schema reference.
...
1. **Test locally** → `pytest em-agent-workspace/tests/ -v`
2. **Run workflow** → `python em-agent-workspace/agents/workflow.py`
