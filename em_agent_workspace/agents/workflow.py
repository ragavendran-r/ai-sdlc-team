...
def compile_em_workflow_web() -> object:
    """
    Compile the EM workflow for the web interface.

    Uses a MemorySaver checkpointer and interrupts before the human_checkpoint
    node so the web layer can inject the sprint approval decision via
    update_state, then resume.
    """
