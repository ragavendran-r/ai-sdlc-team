...
pytest po-agent-workspace/tests/test_nodes.py::TestStoryGeneration -v

# Run with coverage
pytest po-agent-workspace/tests/test_nodes.py --cov=po_agent_workspace
```
...
All schemas defined in `team-contracts/schemas/`:
...
- **Handoff:** User stories feed directly into `em-agent-workspace`
...
1. **Test locally** → `pytest po-agent-workspace/tests/ -v`
2. **Run workflow** → `python po-agent-workspace/agents/workflow.py`
