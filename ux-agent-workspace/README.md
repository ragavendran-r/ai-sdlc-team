...
```bash
# Run all tests
pytest ux-agent-workspace/tests/test_nodes.py -v

# Run specific test class
pytest ux-agent-workspace/tests/test_nodes.py::TestPersonaAgent -v

# Run with coverage
pytest ux-agent-workspace/tests/test_nodes.py --cov=ux_agent_workspace
```
...
Six new schemas created for UX workflow (in `team-contracts/schemas/`):
...
1. **Test locally** → `pytest ux-agent-workspace/tests/ -v`
2. **Run workflow** → `python ux-agent-workspace/agents/workflow.py`
